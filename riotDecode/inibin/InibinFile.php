		...
		if($this->values === null) {
			...
			$this->values = $this->decodeSegments($content, [
				...
				0x0002 => function($index, $key, &$values) use (&$content, &$oldstyleOffset) {
					$values[$key] = $this->readFromStream($content, 'f');
				},
				...
				0x0080 => function($index, $key, &$values) use (&$content, &$oldstyleOffset) {
					$values[$key] = [$this->readFromStream($content, 'f'), $this->readFromStream($content, 'f'), $this->readFromStream($content, 'f')];
				},
				...
				0x0800 => function($index, $key, &$values) use (&$content, &$oldstyleOffset) {
					$values[$key] = [$this->readFromStream($content, 'f'), $this->readFromStream($content, 'f'), $this->readFromStream($content, 'f'), $this->readFromStream($content, 'f')];
				},
